...
class CTCEmotionModel():
	def __init__(self,model_path='ctc_emotion_model.h5'):
		self.model = None
	...
	def predict(self,features):
		...
		emotion_result_id = decode_function[0][0].eval(session=self.session)[0][0]
		emotion_result = self.emotions[emotion_result_id]
		return emotion_result

def wav_to_features_from_wav_path(wav_path):
	'''
...
def wav_to_features_from_wav_array(frames):
...
# model = CTCEmotionModel()
# # read input data,now features still have various lengths
# # features = wav_to_features_from_wav_path('E:\\IEMOCAP_full_release\\Session1\\sentences\\wav\\Ses01F_impro01\\Ses01F_impro01_M004.wav')
